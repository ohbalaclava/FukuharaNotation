...
const A4PortraitLayout = (function () {
  ...
  const pageBreak = (lineNumber) =>
    lineNumber === Config.linesPerPage - 1 || (lineNumber + 1) % Config.linesPerPage === 0;
...
export default function getPDFScore(score) {
  ...
  function addTitle() {
    ...
    doc.text(score.getTitle(), layout.title.x, layout.title.y, layout.title.opts);
  }

  function addAuthor() {
    doc.setFontSize(layout.author.fontSize);
    doc.text(score.getAuthor(), layout.author.x, layout.author.y, layout.author.opts);
  }

  function addNotes() {
    doc.setFontSize(layout.notes.fontSize);
    doc.text(score.getNotes(), layout.notes.x, layout.notes.y, layout.notes.opts);
  }
  ...
  function addAccidental(accidental, _markIndex) {
    doc.addImage(
